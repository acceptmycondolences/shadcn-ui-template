...
import { classNames, useIsMobile } from '~/shared/lib'
import {
  Button,
  Input,
  Separator,
...
function Sidebar({
  ...
  if (collapsible === 'none') {
    return (
      <div
        className={classNames('flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground', className)}
        data-slot="sidebar"
        {...props}
      >
        {children}
...
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={classNames('flex flex-col gap-2 p-2 [--radius:var(--radius-xl)]', className)}
      data-sidebar="header"
...
function SidebarInput({ className, ...props }: React.ComponentProps<typeof Input>) {
  return (
    <Input
      className={classNames('h-8 w-full bg-input/50 shadow-none', className)}
      data-sidebar="input"
...
function SidebarMenu({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      className={classNames('flex w-full min-w-0 flex-col gap-0.5', className)}
      data-sidebar="menu"
...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      className={classNames(sidebarMenuButtonVariants({ size, variant }), className)}
...
function SidebarMenuSkeleton({
  ...
  return (
    <div
      className={classNames('flex h-8 items-center gap-2 rounded-xl px-2', className)}
