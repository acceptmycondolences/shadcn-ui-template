...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div className={classNames('font-medium', labelClassName)}>{labelFormatter(value, payload)}</div>
    }
                ...
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    {itemConfig?.icon ? (
                      ...
                      !hideIndicator && (
                        <div
                          className={classNames('shrink-0 rounded-xs border-(--color-border) bg-(--color-bg)', {
    ...
    >
      {payload
        ...
        .map((item, index) => {
          ...
          return (
            <div
              className={classNames('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground')}
              key={index}
            >
