...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
  return (
    <div className="relative w-full overflow-x-auto" data-slot="table-container">
      <table className={classNames('w-full caption-bottom text-sm', className)} data-slot="table" {...props} />
    </div>
  )
}
...
function TableCell({ className, ...props }: React.ComponentProps<'td'>) {
  return (
    <td
      className={classNames('p-3 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0', className)}
...
function TableHeader({ className, ...props }: React.ComponentProps<'thead'>) {
  return <thead className={classNames('[&_tr]:border-b', className)} data-slot="table-header" {...props} />
}
